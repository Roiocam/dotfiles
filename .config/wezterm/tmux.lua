...
local function normalize_split_size(size)
...
local function split_empty_pane(pane, direction, size, cwd)
	return pane:split({
		direction = direction,
		size = normalize_split_size(size),
		cwd = cwd,
	})
end

local function apply_tab_layout(root_pane, tab_config, project)
	if tab_config.layout == "right_stack" then
		local right_pane = split_empty_pane(root_pane, "Right", tab_config.right_size, project.cwd)
		split_empty_pane(right_pane, "Bottom", tab_config.bottom_size, project.cwd)
		root_pane:activate()
	end
end
...
local function setup_project_tabs(project)
	wezterm.time.call_after(0.3, function()
		...
		for i, tab_config in ipairs(tabs) do
			...
			apply_tab_layout(pane, tab_config, project)
		end
